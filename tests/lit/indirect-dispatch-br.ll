...
; CHECK: entry:
; CHECK: %obf.idis.site0.xor0 = xor i64 sub (i64 ptrtoint (ptr blockaddress(@branch_dispatch, %gt) to i64), i64 ptrtoint (ptr blockaddress(@branch_dispatch, %gt) to i64)), {{-?[0-9]+}}
; CHECK: %obf.idis.site0.tok0 = add i64 %obf.idis.site0.rot0, {{-?[0-9]+}}
; CHECK: %obf.idis.site0.xor1 = xor i64 sub (i64 ptrtoint (ptr blockaddress(@branch_dispatch, %le) to i64), i64 ptrtoint (ptr blockaddress(@branch_dispatch, %gt) to i64)), {{-?[0-9]+}}
