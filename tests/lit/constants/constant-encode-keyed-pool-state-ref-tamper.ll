...
; RUN: %lli %t
; RUN: %python %S/../Inputs/tamper_string_auth_ir.py %t auto state-ref
; RUN: not --crash %lli %t

@lut = private constant [8 x i32] [i32 7, i32 11, i32 13, i32 17, i32 19, i32 23, i32 29, i32 31], align 4

define i32 @table_user(i32 %idx) {
entry:
  %wide = sext i32 %idx to i64
  %slot = getelementptr inbounds [8 x i32], ptr @lut, i64 0, i64 %wide
