...
; CHECK-DAG: call ptr @__obf_family_{{.*}}(ptr {{.*}}, i32 %obf.state, i32 {{-?[0-9][0-9][0-9][0-9][0-9]+}})
; CHECK-DAG: call ptr @__obf_family_{{.*}}(ptr {{.*}}, i32 %obf.state, i32 {{-?[0-9][0-9][0-9][0-9][0-9]+}})
; CHECK-LABEL: define internal ptr @__obf_family_{{.*}}(ptr %desc, i32 %cfg_state, i32 %expected_state) {
