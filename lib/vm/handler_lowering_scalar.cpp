...
llvm::Value* emit_integer_icmp(llvm::IRBuilder<>& builder,
                               opcode predicate,
                               llvm::Value* lhs,
                               llvm::Value* rhs,
...
std::uint64_t mix_handler_family_shape_seed(const rewrite_function_context& context,
  ...
  mixed = mix_seed(mixed,
                   (static_cast<std::uint64_t>(opcode_to_index(instruction.op)) + 1) *
                       0x9e3779b97f4a7c15ULL);
  mixed = mix_seed(
      mixed,
      (static_cast<std::uint64_t>(get_physical_opcode(context.opcode_map, instruction.op)) + 1) *
          0xbf58476d1ce4e5b9ULL);
  mixed =
      mix_seed(mixed, (static_cast<std::uint64_t>(instruction_index) + 1) * 0x94d049bb133111ebULL);
  mixed = mix_seed(
      mixed, (static_cast<std::uint64_t>(instruction.operands.size()) + 1) * 0xd6e8feb86659fd93ULL);
  mixed = mix_seed(mixed,
                   (static_cast<std::uint64_t>(
                        instruction.result_slot == invalid_slot ? 0 : instruction.result_slot + 1) +
                    1) *
                       0x369dea0f31a53f85ULL);
  mixed = mix_seed(mixed,
                   (static_cast<std::uint64_t>(instruction.immediate) + 1) * 0xda942042e4dd58b5ULL);
  return mixed;
}
...
Shape select_handler_family_shape(const rewrite_function_context& context,
  ...
  const std::uint64_t mixed =
      mix_handler_family_shape_seed(context, instruction, instruction_index, salt, family_salt);
  return static_cast<Shape>(mixed % variant_count);
}

llvm::AllocaInst*
create_handler_temp_slot(llvm::IRBuilder<>& builder, llvm::Type* type, llvm::StringRef name) {
  llvm::BasicBlock* block = builder.GetInsertBlock();
  llvm::Function* function = block != nullptr ? block->getParent() : nullptr;
  if (function == nullptr || type == nullptr) { return nullptr; }
...
void apply_integer_binary_flags(llvm::Instruction* instruction, std::uint32_t flags) {
  if (instruction == nullptr) { return; }
  auto* binary = llvm::dyn_cast<llvm::BinaryOperator>(instruction);
  if (binary == nullptr) { return; }
  if (has_instruction_flag(flags, instruction_flag_nsw)) { binary->setHasNoSignedWrap(); }
  if (has_instruction_flag(flags, instruction_flag_nuw)) { binary->setHasNoUnsignedWrap(); }
}

llvm::Value* apply_scalar_handler_shape(llvm::IRBuilder<>& builder,
                                        scalar_handler_shape shape,
                                        llvm::Value* result) {
  if (result == nullptr || !result->getType()->isIntegerTy()) { return result; }

  const llvm::StringRef marker = scalar_shape_marker(shape);
  if (shape == scalar_handler_shape::direct) {
    if (result->hasName()) { result->setName(marker); }
    return result;
  }
...
llvm::Value* apply_compare_handler_shape(llvm::IRBuilder<>& builder,
                                         ...
                                         llvm::Value* result) {
  if (result == nullptr || !result->getType()->isIntegerTy(1)) { return result; }
  ...
  switch (shape) {
    ...
    case compare_handler_shape::select_materialized:
      return builder.CreateSelect(result, builder.getTrue(), builder.getFalse(), marker);
    case compare_handler_shape::inverted_predicate:
...
llvm::Value* emit_icmp_result(llvm::IRBuilder<>& builder,
  ...
  llvm::Value* lhs = materialize_value(builder,
  ...
  llvm::Value* rhs = materialize_value(builder,
                                       function_context.slot_allocas,
                                       context.current_slot_mapping,
                                       function_context.program,
  ...
  if (shape == compare_handler_shape::inverted_predicate) {
    llvm::Value* inverted = builder.CreateICmp(
        llvm::CmpInst::getInversePredicate(icmp_predicate_for_opcode(instruction.op)),
        lhs,
        rhs,
        "obf.vm.icmp.inv");
  ...
  if (lhs->getType()->isIntegerTy() && lhs->getType() == rhs->getType()) {
    result = emit_integer_icmp(builder,
                               instruction.op,
                               lhs,
                               rhs,
                               function_context.mba_context,
    ...
    result = builder.CreateICmp(icmp_predicate_for_opcode(instruction.op), lhs, rhs, "obf.vm.icmp");
  }
...
llvm::CmpInst::Predicate icmp_predicate_for_opcode(opcode op) {
  if (!is_icmp_opcode(op)) { llvm_unreachable("opcode is not an icmp predicate"); }
...
llvm::CmpInst::Predicate fcmp_predicate_for_opcode(opcode op) {
  if (!is_fcmp_opcode(op)) { llvm_unreachable("opcode is not an fcmp predicate"); }
...
llvm::Value* emit_integer_nonzero_test(llvm::IRBuilder<>& builder,
                                       llvm::Value* value,
                                       const mba::builder_context& mba_context,
                                       std::uint64_t salt) {
  auto* integer_type = llvm::cast<llvm::IntegerType>(value->getType());
  llvm::Value* negated = mba::create_sub(builder,
                                         llvm::ConstantInt::get(integer_type, 0),
                                         value,
                                         mba_context,
                                         salt + 1,
                                         "obf.vm.icmp.nz.neg");
...
llvm::Value* emit_integer_unsigned_lt(llvm::IRBuilder<>& builder,
                                      llvm::Value* lhs,
                                      llvm::Value* rhs,
                                      const mba::builder_context& mba_context,
                                      std::uint64_t salt) {
  auto* integer_type = llvm::cast<llvm::IntegerType>(lhs->getType());
  auto* wide_type = llvm::IntegerType::get(builder.getContext(), integer_type->getBitWidth() + 1);
  ...
  llvm::Value* diff =
      mba::create_sub(builder, lhs_ext, rhs_ext, mba_context, salt + 1, "obf.vm.icmp.ult.diff");
  llvm::Value* borrow =
      builder.CreateLShr(diff,
  ...
  switch (predicate) {
    ...
    case opcode::icmp_ne: {
      llvm::Value* difference =
          mba::create_xor(builder, lhs, rhs, mba_context, salt + 1, "obf.vm.icmp.eq.diff");
      llvm::Value* nonzero = emit_integer_nonzero_test(builder, difference, mba_context, salt + 2);
    ...
    case opcode::icmp_ugt:
      ...
      break;
    case opcode::icmp_ule:
      result = builder.CreateXor(emit_integer_unsigned_lt(builder, rhs, lhs, mba_context, salt + 5),
                                 builder.getTrue(),
                                 "obf.vm.icmp.ule");
      break;
    case opcode::icmp_uge:
      result = builder.CreateXor(emit_integer_unsigned_lt(builder, lhs, rhs, mba_context, salt + 6),
                                 builder.getTrue(),
                                 "obf.vm.icmp.uge");
      break;
    case opcode::icmp_slt:
    ...
    case opcode::icmp_sge: {
      ...
      llvm::Value* lhs_biased =
          mba::create_xor(builder, lhs, sign_mask, mba_context, salt + 7, "obf.vm.icmp.signed.lhs");
      llvm::Value* rhs_biased =
          mba::create_xor(builder, rhs, sign_mask, mba_context, salt + 8, "obf.vm.icmp.signed.rhs");
      switch (predicate) {
        case opcode::icmp_slt:
          result = emit_integer_unsigned_lt(builder, lhs_biased, rhs_biased, mba_context, salt + 9);
          break;
        case opcode::icmp_sgt:
          result =
              emit_integer_unsigned_lt(builder, rhs_biased, lhs_biased, mba_context, salt + 10);
          break;
        case opcode::icmp_sle:
          result = builder.CreateXor(
              emit_integer_unsigned_lt(builder, rhs_biased, lhs_biased, mba_context, salt + 11),
              builder.getTrue(),
              "obf.vm.icmp.sle");
          break;
        case opcode::icmp_sge:
          result = builder.CreateXor(
              emit_integer_unsigned_lt(builder, lhs_biased, rhs_biased, mba_context, salt + 12),
              builder.getTrue(),
              "obf.vm.icmp.sge");
          break;
        default:
...
llvm::Value* emit_integer_sign_recovery(llvm::IRBuilder<>& builder,
  ...
  if (source_bit_width >= destination_type->getBitWidth()) { return widened_value; }

  llvm::Value* shift_amount =
      llvm::ConstantInt::get(destination_type, destination_type->getBitWidth() - source_bit_width);
  llvm::Value* shifted =
...
llvm::Value* emit_integer_zext(llvm::IRBuilder<>& builder,
                               llvm::Value* operand,
  ...
  llvm::Constant* source_bias = llvm::ConstantInt::get(
      source_type,
      llvm::APInt::getOneBitSet(source_type->getBitWidth(), source_type->getBitWidth() - 1));
  llvm::Value* biased =
      mba::create_xor(builder, operand, source_bias, mba_context, salt + 1, "obf.vm.zext.bias");
  llvm::Value* widened = builder.CreateZExt(biased, destination_type, "obf.vm.zext.wide");
  widened = mba::create_xor(builder,
                            widened,
  ...
  llvm::Constant* destination_bias = llvm::ConstantInt::get(
      destination_type,
      llvm::APInt::getOneBitSet(destination_type->getBitWidth(), source_type->getBitWidth() - 1));
  return mba::create_add(
      builder, signed_value, destination_bias, mba_context, salt + 3, "obf.vm.zext");
}

llvm::Value* emit_integer_sext(llvm::IRBuilder<>& builder,
                               llvm::Value* operand,
                               llvm::IntegerType* destination_type,
                               const mba::builder_context& mba_context,
                               std::uint64_t salt) {
  auto* source_type = llvm::cast<llvm::IntegerType>(operand->getType());
  llvm::Value* widened = builder.CreateZExt(operand, destination_type, "obf.vm.sext.wide");
  widened = mba::create_xor(builder,
                            widened,
                            llvm::ConstantInt::get(destination_type, 0),
                            mba_context,
                            salt + 1,
                            "obf.vm.sext.mix");
  return emit_integer_sign_recovery(builder, widened, source_type->getBitWidth(), "obf.vm.sext");
}

llvm::Value* emit_integer_trunc(llvm::IRBuilder<>& builder,
                                llvm::Value* operand,
                                llvm::IntegerType* destination_type,
                                const mba::builder_context& mba_context,
                                std::uint64_t salt) {
  ...
  llvm::Value* masked = builder.CreateAnd(
      ...
      llvm::ConstantInt::get(
          source_type,
          llvm::APInt::getLowBitsSet(source_type->getBitWidth(), destination_type->getBitWidth())),
      "obf.vm.trunc.mask");
  if (destination_type->isIntegerTy(1)) {
    masked = mba::create_xor(builder,
                             masked,
                             llvm::ConstantInt::get(source_type, 0),
                             mba_context,
                             salt + 1,
                             "obf.vm.trunc.mix");
...
llvm::Value* emit_integer_cast(llvm::IRBuilder<>& builder,
                               opcode cast_opcode,
                               llvm::Value* operand,
  ...
  auto* destination_integer_type = llvm::dyn_cast<llvm::IntegerType>(destination_type);
  if (source_type == nullptr || destination_integer_type == nullptr) { return nullptr; }

  switch (cast_opcode) {
    case opcode::trunc:
      if (source_type->getBitWidth() > destination_integer_type->getBitWidth()) {
        return emit_integer_trunc(
            builder, operand, destination_integer_type, mba_context, salt + 1);
      }
      break;
    case opcode::zext:
      if (source_type->getBitWidth() < destination_integer_type->getBitWidth()) {
        return emit_integer_zext(builder, operand, destination_integer_type, mba_context, salt + 2);
      }
      break;
    case opcode::sext:
      if (source_type->getBitWidth() < destination_integer_type->getBitWidth()) {
        return emit_integer_sext(builder, operand, destination_integer_type, mba_context, salt + 3);
      }
      break;
    default:
      break;
...
llvm::Value* emit_binary(llvm::IRBuilder<>& builder,
  ...
  if (is_polymorphic_integer_binary_opcode(instruction.op) && lhs->getType()->isIntegerTy() &&
      lhs->getType() == rhs->getType()) {
    const scalar_handler_shape shape = select_scalar_handler_shape(
        bytecode_seed, instruction.op, get_physical_opcode(opcode_map, instruction.op), salt);
    result = emit_current_integer_binary(builder,
                                         instruction.op,
                                         lhs,
                                         rhs,
                                         instruction.flags,
  ...
  switch (instruction.op) {
    case opcode::add:
      if (!has_instruction_flag(instruction.flags, instruction_flag_nsw) &&
        ...
        result = mba::create_add(builder, lhs, rhs, mba_context, salt + 3, "obf.vm.add");
    ...
    case opcode::sub:
      if (!has_instruction_flag(instruction.flags, instruction_flag_nsw) &&
          !has_instruction_flag(instruction.flags, instruction_flag_nuw)) {
        result = mba::create_sub(builder, lhs, rhs, mba_context, salt + 4, "obf.vm.sub");
      } else if (select_handler_variant(instruction.op, opaque_seed_base, salt) == 0) {
    ...
    case opcode::or_op:
      ...
      break;
    case opcode::xor_op:
      result = mba::create_xor(builder, lhs, rhs, mba_context, salt + 5, "obf.vm.xor");
      break;
    case opcode::fadd:
...
llvm::Value* emit_cast(llvm::IRBuilder<>& builder,
                       ...
                       const mba::builder_context& mba_context,
                       std::uint64_t salt) {
  if (!is_cast_opcode(instruction.op)) { llvm_unreachable("opcode is not a cast opcode"); }

  llvm::Type* const destination_type =
      const_cast<llvm::Type*>(program.slots[instruction.result_slot].type);
  const llvm::Type* const source_type = value_ref_type(program, instruction.operands[0]);

  llvm::Value* operand = nullptr;
  const auto materialize_operand = [&]() -> llvm::Value* {
    if (operand == nullptr) {
      operand = materialize_value(builder,
                                  slot_allocas,
                                  slot_mapping,
                                  program,
                                  instruction.operands[0],
  ...
  if (instruction.op == opcode::ptr_to_int && source_type->isPointerTy() &&
    ...
    if (llvm::Value* carrier = materialize_pointer_carrier_from_value_ref(builder,
                                                                          slot_allocas,
                                                                          slot_mapping,
                                                                          program,
                                                                          instruction.operands[0],
      ...
      return emit_unsigned_integer_width_cast(
          builder, carrier, llvm::cast<llvm::IntegerType>(destination_type), mba_context, salt + 3);
    }
  }
  ...
  if (instruction.op == opcode::int_to_ptr && destination_type->isPointerTy()) {
    ...
    if (auto* carrier_type = get_pointer_carrier_type(builder, destination_type)) {
      if (llvm::Value* carrier = emit_unsigned_integer_width_cast(
        ...
        carrier = mba::entangle_value(
            builder, carrier, mba_context, salt ^ 0x5606ULL, "obf.vm.inttoptr.carrier");
        return builder.CreateIntToPtr(carrier, destination_type, "obf.vm.inttoptr");
      }
    }
  }
    ...
    if (llvm::Value* carrier = materialize_pointer_carrier_from_value_ref(builder,
      ...
      if (auto* carrier_type = get_pointer_carrier_type(builder, destination_type)) {
        if (llvm::Value* normalized = emit_unsigned_integer_width_cast(
          ...
          return builder.CreateIntToPtr(normalized, destination_type, "obf.vm.bitcast");
        }
      }
    }
  }
  ...
  if (llvm::Value* integer_cast = emit_integer_cast(
          builder, instruction.op, materialized_operand, destination_type, mba_context, salt + 2)) {
    return integer_cast;
  }

  switch (instruction.op) {
    case opcode::trunc:
      return builder.CreateTrunc(materialized_operand, destination_type, "obf.vm.trunc");
    case opcode::zext:
      return builder.CreateZExt(materialized_operand, destination_type, "obf.vm.zext");
    case opcode::sext:
      return builder.CreateSExt(materialized_operand, destination_type, "obf.vm.sext");
    case opcode::fp_trunc:
      return builder.CreateFPTrunc(materialized_operand, destination_type, "obf.vm.fptrunc");
    case opcode::fp_ext:
      return builder.CreateFPExt(materialized_operand, destination_type, "obf.vm.fpext");
    case opcode::ui_to_fp:
      return builder.CreateUIToFP(materialized_operand, destination_type, "obf.vm.uitofp");
    case opcode::si_to_fp:
      return builder.CreateSIToFP(materialized_operand, destination_type, "obf.vm.sitofp");
    case opcode::fp_to_ui:
      return builder.CreateFPToUI(materialized_operand, destination_type, "obf.vm.fptoui");
    case opcode::fp_to_si:
      return builder.CreateFPToSI(materialized_operand, destination_type, "obf.vm.fptosi");
    case opcode::ptr_to_int:
      return builder.CreatePtrToInt(materialized_operand, destination_type, "obf.vm.ptrtoint");
    case opcode::int_to_ptr:
      return builder.CreateIntToPtr(materialized_operand, destination_type, "obf.vm.inttoptr");
    case opcode::bitcast:
      return builder.CreateBitCast(materialized_operand, destination_type, "obf.vm.bitcast");
    case opcode::addrspace_cast:
      return builder.CreateAddrSpaceCast(
          materialized_operand, destination_type, "obf.vm.addrspacecast");
    default:
...
llvm::FastMathFlags decode_fast_math_flags(std::uint32_t flags) {
  ...
  fast_math.setAllowReassoc(has_instruction_flag(flags, instruction_flag_fast_reassoc));
  ...
  fast_math.setNoSignedZeros(has_instruction_flag(flags, instruction_flag_fast_nsz));
  fast_math.setAllowReciprocal(has_instruction_flag(flags, instruction_flag_fast_arcp));
  fast_math.setAllowContract(has_instruction_flag(flags, instruction_flag_fast_contract));
  fast_math.setApproxFunc(has_instruction_flag(flags, instruction_flag_fast_afn));
  if (has_instruction_flag(flags, instruction_flag_fast_fast)) { fast_math.setFast(); }
  return fast_math;
}
...
std::uint32_t select_handler_variant(opcode op,
  ...
  return static_cast<std::uint32_t>(
      mix_seed(
          seed_base,
          salt ^ ((static_cast<std::uint64_t>(opcode_to_index(op)) + 1) * 0x9e3779b97f4a7c15ULL)) %
      variant_count);
}
...
scalar_handler_shape select_scalar_handler_shape(std::uint64_t seed_base,
  ...
  mixed = mix_seed(mixed,
                   (static_cast<std::uint64_t>(opcode_to_index(op)) + 1) * 0x9e3779b97f4a7c15ULL);
  mixed =
      mix_seed(mixed, (static_cast<std::uint64_t>(physical_opcode) + 1) * 0xbf58476d1ce4e5b9ULL);
  return static_cast<scalar_handler_shape>(mixed % 3);
}

compare_handler_shape select_compare_handler_shape(const rewrite_function_context& context,
                                                   const micro_instruction& instruction,
                                                   std::size_t instruction_index,
                                                   std::uint64_t salt) {
...
branch_handler_shape select_branch_handler_shape(const rewrite_function_context& context,
                                                 const micro_instruction& instruction,
                                                 std::size_t instruction_index,
                                                 std::uint64_t salt) {
...
memory_handler_shape select_memory_handler_shape(const rewrite_function_context& context,
                                                 const micro_instruction& instruction,
                                                 std::size_t instruction_index,
                                                 std::uint64_t salt) {
...
gep_handler_shape select_gep_handler_shape(const rewrite_function_context& context,
  ...
  return select_handler_family_shape<gep_handler_shape>(
      context, instruction, instruction_index, salt, 0x6765707368617065ULL, 5);
}

call_handler_shape select_call_handler_shape(const rewrite_function_context&,
...
return_handler_shape select_return_handler_shape(const rewrite_function_context& context,
                                                 const micro_instruction& instruction,
                                                 std::size_t instruction_index,
                                                 std::uint64_t salt) {
...
llvm::Value* emit_unsigned_integer_width_cast(llvm::IRBuilder<>& builder,
                                              llvm::Value* operand,
                                              llvm::IntegerType* destination_type,
                                              const mba::builder_context& mba_context,
                                              std::uint64_t salt) {
  auto* source_type = llvm::dyn_cast<llvm::IntegerType>(operand->getType());
  if (source_type == nullptr || destination_type == nullptr) { return nullptr; }

  if (source_type == destination_type) { return operand; }

  if (source_type->getBitWidth() < destination_type->getBitWidth()) {
    return emit_integer_zext(builder, operand, destination_type, mba_context, salt + 1);
  }

  if (source_type->getBitWidth() > destination_type->getBitWidth()) {
    return emit_integer_trunc(builder, operand, destination_type, mba_context, salt + 2);
  }
...
bool lower_scalar_instruction(llvm::IRBuilder<>& builder,
  ...
  switch (instruction.op) {
    ...
    case opcode::bitcast:
    case opcode::addrspace_cast:
      if (select_handler_variant(
              instruction.op, function_context.opaque_seed_base, 0xc800 + instruction_index) == 0) {
        finish_value(builder,
                     context,
                     emit_cast(builder,
                               function_context.slot_allocas,
        ...
        emit_in_helper_block(
            builder, context, "vm.cast.exec.", [&](llvm::IRBuilder<>& helper_builder) {
              finish_value_in_builder(helper_builder,
                                      context,
                                      emit_cast(helper_builder,
                                                function_context.slot_allocas,
                                                context.current_slot_mapping,
                                                function_context.program,
                                                instruction,
    ...
    case opcode::freeze:
      finish_value(builder,
                   context,
                   builder.CreateFreeze(materialize_value(builder,
                                                          function_context.slot_allocas,
                                                          context.current_slot_mapping,
                                                          function_context.program,
                                                          instruction.operands[0],
    ...
    case opcode::icmp_sle: {
      const compare_handler_shape shape = select_compare_handler_shape(
          function_context, instruction, instruction_index, 0xe700 + instruction_index);
      const auto emit_compare = [&](llvm::IRBuilder<>& compare_builder) {
        return emit_icmp_result(compare_builder, context, shape, 0xe000 + instruction_index * 32);
      };
      ...
      if (value_ref_type(function_context.program, instruction.operands[0])->isIntegerTy() &&
        ...
        emit_in_helper_block(
            builder, context, "vm.icmp.exec.", [&](llvm::IRBuilder<>& helper_builder) {
              finish_value_in_builder(helper_builder, context, emit_compare(helper_builder));
            });
      }
      return true;
    }
    ...
    case opcode::fcmp_true: {
      const compare_handler_shape shape = select_compare_handler_shape(
          function_context, instruction, instruction_index, 0xf700 + instruction_index);
      if (select_handler_variant(
              instruction.op, function_context.opaque_seed_base, 0xf800 + instruction_index) == 0) {
        finish_value(builder,
                     context,
                     emit_fcmp_result(builder, context, shape, 0xf000 + instruction_index * 32));
      } else {
        emit_in_helper_block(
            builder, context, "vm.fcmp.exec.", [&](llvm::IRBuilder<>& helper_builder) {
              finish_value_in_builder(
                  helper_builder,
                  context,
                  emit_fcmp_result(
                      helper_builder, context, shape, 0xf000 + instruction_index * 32));
            });
      }
      return true;
    }
    ...
    case opcode::select:
      if (instruction.result_slot != invalid_slot &&
          ...
          value_ref_type(function_context.program, instruction.operands[0])->isIntegerTy(1)) {
        auto* true_block =
        ...
        llvm::Value* true_value = materialize_value(true_builder,
                                                    function_context.slot_allocas,
                                                    context.current_slot_mapping,
                                                    function_context.program,
                                                    instruction.operands[1],
        ...
        llvm::Value* false_value = materialize_value(false_builder,
                                                     function_context.slot_allocas,
                                                     context.current_slot_mapping,
                                                     function_context.program,
        ...
        finish_value(builder,
                     context,
                     builder.CreateSelect(materialize_value(builder,
                                                            function_context.slot_allocas,
                                                            context.current_slot_mapping,
                                                            function_context.program,
                                                            instruction.operands[0],
