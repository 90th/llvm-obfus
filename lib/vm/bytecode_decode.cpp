...
llvm::Value *fetch_byte(llvm::IRBuilder<> &builder,
  ...
  if (bytecode_base == nullptr) {
    bytecode_base = context.bytecode_global;
  }
...
decoded_metadata_span_result decode_metadata_span(
  ...
  llvm::Value *bytecode_base = materialize_pointer_value(
      builder, context.bytecode_global, context.opaque_seed_slot,
      context.opaque_seed_base, context.mba_context, salt ^ 0x5f0fULL);
  if (bytecode_base == nullptr) {
    bytecode_base = context.bytecode_global;
  }

  llvm::Value *state = builder.CreateLoad(builder.getInt64Ty(), context.state_slot,
...
void emit_instruction_integrity_probes(llvm::IRBuilder<> &builder,
  ...
  if (layout.integrity_probe_range == 0 ||
      context.function_context.bytecode_global == nullptr) {
    return;
  }
  ...
  for (std::uint32_t probe = 0; probe < num_probes; ++probe) {
    const std::uint32_t probe_offset = static_cast<std::uint32_t>(
        mix_seed(context.function_context.bytecode_seed,
                 static_cast<std::uint64_t>(context.instruction_index) * 0x1337ULL +
                     probe + 1) %
        layout.integrity_probe_range);
    llvm::Value *bytecode_base = materialize_pointer_value(
        builder, context.function_context.bytecode_global,
    ...
    if (bytecode_base == nullptr) {
      bytecode_base = context.function_context.bytecode_global;
    }
    llvm::Value *byte_ptr = builder.CreateInBoundsGEP(
        context.function_context.bytecode_global->getValueType(), bytecode_base,
    ...
    auto *integrity_state = builder.CreateLoad(
        builder.getInt64Ty(), context.function_context.state_slot,
