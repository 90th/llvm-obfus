...
bool has_non_annotation_address_taken(const llvm::Function& function) {
  for (const llvm::Use& use : function.uses()) {
    if (const auto* call = llvm::dyn_cast<llvm::CallBase>(use.getUser())) {
      if (call->isCallee(&use)) { continue; }
    }

    if (!is_annotation_user(use.getUser())) { return true; }
  }
...
bool references_string_literal(const llvm::Instruction& instruction) {
  for (const llvm::Use& use : instruction.operands()) {
    ...
    if (global == nullptr || !global->hasInitializer()) { continue; }

    const auto* data = llvm::dyn_cast<llvm::ConstantDataSequential>(global->getInitializer());
    if (data != nullptr && data->isString()) { return true; }
  }
...
std::size_t count_cfg_edges(const llvm::Function& function) {
  ...
  for (const llvm::BasicBlock& block : function) { edge_count += llvm::succ_size(&block); }
...
function_features collect_function_features(const llvm::Function& function) {
  ...
  if (features.is_declaration) { return features; }

  features.basic_block_count = function.size();

  llvm::SmallVector<std::pair<const llvm::BasicBlock*, const llvm::BasicBlock*>, 8> backedges;
  ...
  features.cyclomatic_complexity =
      edge_count >= features.basic_block_count ? edge_count - features.basic_block_count + 2 : 1;

  for (const llvm::BasicBlock& block : function) {
    features.has_exception_edges = features.has_exception_edges || block.isEHPad();

    for (const llvm::Instruction& instruction : block) {
      ++features.instruction_count;

      if (llvm::isa<llvm::InvokeInst>(instruction)) { features.has_exception_edges = true; }

      if (const auto* call = llvm::dyn_cast<llvm::CallBase>(&instruction)) {
        ++features.call_count;

        if (call->isInlineAsm()) { features.has_inline_asm = true; }

        if (const llvm::Function* callee = call->getCalledFunction()) {
          if (callee == &function) { features.is_recursive = true; }
        }
      }

      if (instruction.getType()->isVectorTy()) { features.has_vector_ops = true; }
      ...
      if (references_string_literal(instruction)) { ++features.string_ref_count; }
    }
  }
