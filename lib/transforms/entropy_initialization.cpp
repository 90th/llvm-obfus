...
std::uint64_t GetSeededWord(std::uint64_t seed_override, std::uint64_t current_word) {
  std::uint64_t seeded_word =
      mix_entropy_seed(seed_override, stable_hash_string(kEntropyAnchorName));
  seeded_word = mix_entropy_seed(seeded_word, 0x454e54524f505901ULL);
  if (seeded_word == 0) { seeded_word = 0x6a09e667f3bcc909ULL; }
  if (seeded_word == current_word) {
    seeded_word = mix_entropy_seed(seeded_word, 0x454e54524f505902ULL);
    if (seeded_word == 0) { seeded_word = 0xbb67ae8584caa73bULL; }
  }
  return seeded_word;
}
...
bool RunEntropyInitialization(llvm::Module& module, std::uint64_t seed_override) {
  llvm::GlobalVariable* entropy_anchor = module.getNamedGlobal(kEntropyAnchorName);
  ...
  if (const auto* integer_initializer = llvm::dyn_cast<llvm::ConstantInt>(initializer)) {
  ...
  const std::uint64_t random_word =
      seed_override != 0 ? GetSeededWord(seed_override, current_word) : [&]() {
        ...
        do { fresh_word = GetSecureRandomWord(); } while (fresh_word == current_word);
        return fresh_word;
      }();
