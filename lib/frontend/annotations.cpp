...
function_annotation_map collect_function_annotations(const llvm::Module &module) {
  ...
  const llvm::GlobalVariable *annotation_global =
      module.getNamedGlobal("llvm.global.annotations");
  if (annotation_global == nullptr || !annotation_global->hasInitializer()) {
    return annotations;
  }
  ...
  for (const llvm::Use &entry_use : annotation_array->operands()) {
    ...
    const auto *function = llvm::dyn_cast<llvm::Function>(
        entry->getOperand(0)->stripPointerCasts());
    const auto *text_global = llvm::dyn_cast<llvm::GlobalVariable>(
        entry->getOperand(1)->stripPointerCasts());

    if (function == nullptr || text_global == nullptr ||
        !text_global->hasInitializer()) {
      continue;
    }

    const auto *text_data = llvm::dyn_cast<llvm::ConstantDataSequential>(
        text_global->getInitializer());
    if (text_data == nullptr || !text_data->isCString()) {
      continue;
    }
...
const std::string *find_function_annotation(
    const function_annotation_map &annotations, llvm::StringRef function_name) {
