...
const target_rule* find_matching_rule(const obfuscation_config& config,
  ...
  for (const target_rule& rule : config.targets) {
    if (wildcard_match(rule.match, function_name)) { return &rule; }
  }
...
const function_override* find_explicit_override(const obfuscation_config& config,
  ...
  for (const function_override& override : config.overrides) {
    if (override.name == function_name) { return &override; }
  }
...
std::optional<protection_level> derive_minimum_security_floor(const function_features& features) {
  ...
  if (features.string_ref_count > 0) { return protection_level::light; }

  return std::nullopt;
}

bool satisfies_security_floor(protection_level selected, protection_level minimum_floor) {
...
function_policy promote_to_security_floor(function_policy current, protection_level minimum_floor) {
  if (satisfies_security_floor(current.level, minimum_floor)) { return current; }
...
std::optional<protection_level> parse_protection_level(llvm::StringRef text) {
  ...
  if (normalized.starts_with("obf:")) { normalized = normalized.drop_front(4); }

  if (normalized.equals_insensitive("none")) { return protection_level::none; }

  if (normalized.equals_insensitive("light")) { return protection_level::light; }

  if (normalized.equals_insensitive("strong")) { return protection_level::strong; }

  if (normalized.equals_insensitive("vm")) { return protection_level::vm; }

  if (normalized.equals_insensitive("strong_vm")) { return protection_level::strong_vm; }
  ...
  if (features.has_exception_edges || features.has_inline_asm) {
    if (decision.source == policy_source::explicit_override) {
      ...
      if (decision.policy.level == protection_level::vm) {
        ...
        decision.policy = make_function_policy(protection_level::light);
        append_detail(decision.detail, "risky features downgraded strong to light");
      } else if (decision.policy.level == protection_level::strong_vm) {
        decision.policy = make_function_policy(protection_level::light);
        append_detail(decision.detail, "risky features downgraded strong_vm to light");
      }
