...
gep_handler_shape select_gep_handler_shape(const rewrite_function_context& context,
                                           ...
                                           std::size_t instruction_index,
                                           std::uint64_t salt);
call_handler_shape select_call_handler_shape(const rewrite_function_context& context,
                                             const micro_instruction& instruction,
                                             std::size_t instruction_index,
                                             std::uint64_t salt);
...
vm_dispatcher_shape
select_dispatcher_shape(std::uint64_t seed_base, std::uint64_t salt, std::size_t instruction_count);
...
inline llvm::AllocaInst*
create_vm_handler_temp_slot(llvm::IRBuilder<>& builder, llvm::Type* type, const llvm::Twine& name) {
  llvm::BasicBlock* block = builder.GetInsertBlock();
  llvm::Function* function = block != nullptr ? block->getParent() : nullptr;
  if (function == nullptr || type == nullptr || !type->isSized()) { return nullptr; }
...
inline llvm::Value* roundtrip_vm_handler_value(llvm::IRBuilder<>& builder,
  ...
  llvm::AllocaInst* temp = create_vm_handler_temp_slot(builder, value->getType(), name + ".slot");
  if (temp == nullptr) { return value; }

  builder.CreateStore(value, temp);
  return builder.CreateLoad(value->getType(), temp, name);
}

inline llvm::Value* tag_vm_handler_value(llvm::Value* value, llvm::StringRef marker) {
  if (value == nullptr || llvm::isa<llvm::Constant>(value)) { return value; }
...
void emit_in_helper_block(llvm::IRBuilder<>& builder,
                          ...
                          EmitFn&& emit) {
  auto* helper_block =
      llvm::BasicBlock::Create(context.function_context.function.getContext(),
